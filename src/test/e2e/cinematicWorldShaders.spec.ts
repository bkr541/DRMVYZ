...
test.describe('Cinematic Worlds WebGL2 shader smoke', () => {
  test('compiles, links, and renders a non-empty frame for all four worlds', async ({ page }) => {
    ...
    const result = await page.evaluate(({ vertexSource, fragmentSources }) => {
      ...
      const worlds = Object.entries(fragmentSources).map(([name, fragmentSource]) => {
        const vertex = compile(gl.VERTEX_SHADER, vertexSource)
        const fragment = compile(gl.FRAGMENT_SHADER, fragmentSource)
        if (!vertex.shader || !fragment.shader) {
          return { name, compileLog: `${vertex.log}\n${fragment.log}`, linkLog: '', error: -1, energy: 0 }
        }

        const program = gl.createProgram()
        if (!program) return { name, compileLog: '', linkLog: 'createProgram returned null', error: -1, energy: 0 }
        ...
        if (linkLog) {
          ...
          return { name, compileLog: '', linkLog, error: -1, energy: 0 }
        }
        ...
        gl.deleteProgram(program)
        return { name, compileLog: '', linkLog: '', error, energy }
      })
    ...
    expect(result.worlds).toHaveLength(4)
    for (const world of result.worlds) {
      expect(world.compileLog, `${world.name} compile`).toBe('')
      expect(world.linkLog, `${world.name} link`).toBe('')
      expect(world.error, `${world.name} draw`).toBe(0)
      expect(world.energy, `${world.name} output`).toBeGreaterThan(0)
    }
  })
})
