...
export function TransportControls({ isPlaying, currentTime, duration, volume, onPlay, onPause, onStop, onPrev, onNext, onSeek, onVolume, primaryColor, hasTrack }: Props) {
  const pct = duration > 0 ? (currentTime / duration) * 100 : 0

  return (
    <div className="transport">
      <div className="transport-row-top">
        <span className="time-current">{formatTime(currentTime)}</span>
        <div className="seek-bar-wrap" style={{ flex: 1 }}>
          <input type="range" className="seek-bar" min={0} max={duration || 1} step={0.01}
      ...
      <div className="transport-row-btns">
        <div className="transport-btns">
          ...
          <button className="btn-icon" onClick={onStop} disabled={!hasTrack} title="Stop">
            <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><rect x="6" y="6" width="12" height="12"/></svg>
          </button>
          <button className="btn-play" onClick={isPlaying ? onPause : onPlay} disabled={!hasTrack}
        ...
        <div className="volume-wrap">
          <svg viewBox="0 0 24 24" width="12" height="12" fill="currentColor" style={{ opacity: 0.4 }}>
