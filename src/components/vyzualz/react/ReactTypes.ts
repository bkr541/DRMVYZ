...
type NeonLatticePhraseActionMap = Record<NeonLatticePhraseScale, NeonLatticePhraseAction[]>

function makeNeonLatticePhrasePrograms(
  prefix: string,
  actions: NeonLatticePhraseActionMap,
): NeonLatticePhraseProgram[] {
  return ([4, 8, 16, 32] as const).map(phraseBeats => ({
    id: `${prefix}-phrase-${phraseBeats}`,
    name: `${phraseBeats}-Beat Direction`,
    boundary: 'phrase',
    phraseBeats,
    every: 1,
    actions: actions[phraseBeats],
  }))
}
