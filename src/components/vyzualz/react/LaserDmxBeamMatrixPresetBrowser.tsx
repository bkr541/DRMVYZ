...
export function LaserDmxBeamMatrixPresetBrowser({ externalQuery }: { externalQuery?: string } = {}) {
  ...
  // When the shared PresetSearchRow owns the query, it drives filtering here
  // and this browser's own search field is hidden.
  const usesExternalSearch = externalQuery !== undefined
  const effectiveQuery = externalQuery ?? searchQuery
