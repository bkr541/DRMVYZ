...
describe('PixGrid final Reactivity workspace', () => {
  ...
  it('reports unavailable analysis honestly before the renderer publishes a frame', async () => {
    ...
    expect(container.textContent).toContain('PERCEPTUAL RESPONSE METER')
    expect(container.textContent).toContain('WHY PIXGRID IS MOVING')
    expect(container.textContent).toContain('PERFORMANCE STATE')
    ...
    expect(container.querySelector('.rv-pix-grid-perceptual-grid')?.getAttribute('aria-live')).toBe('off')
  })
})
