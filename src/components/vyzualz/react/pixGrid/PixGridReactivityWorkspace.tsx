      ...
      <Collapsible label="RUNTIME DIAGNOSTICS" defaultOpen={false}>
        <div className="rv-pix-grid-diagnostics-grid">
          {[
            ...
            ['Layer graph', renderer?.layerGraphVersion ?? runtime?.layerGraphVersion ?? pixGridState.configuration.layerGraphVersion], ['Canonical migration', (renderer?.canonicalMigrationCompleted ?? runtime?.canonicalMigrationCompleted ?? pixGridState.configuration.canonicalMigrationCompleted) ? 'Complete' : 'Incomplete'],
            ...
            ['Detected lineage', renderer?.migrationDetectedPresetLineage ?? runtime?.migrationDetectedPresetLineage ?? pixGridState.configuration.lastMigration?.detectedPresetLineage ?? 'Unknown'],
            ['Canonical layers added / legacy mapped', `${renderer?.migrationCanonicalLayersAdded ?? runtime?.migrationCanonicalLayersAdded.length ?? pixGridState.configuration.lastMigration?.canonicalLayersAdded?.length ?? 0} / ${renderer?.migrationLegacyLayersMapped ?? runtime?.migrationLegacyLayersMapped.length ?? pixGridState.configuration.lastMigration?.legacyLayersMapped?.length ?? 0}`],
            ['Scene refs repaired', renderer?.migrationSceneReferencesRepaired ?? runtime?.migrationSceneReferencesRepaired ?? pixGridState.configuration.lastMigration?.sceneReferencesRepaired ?? 0],
            ['Empty / missing-layer groups', `${renderer?.migrationEmptyGroupCount ?? runtime?.migrationEmptyGroups.length ?? pixGridState.configuration.lastMigration?.emptyGroups?.length ?? 0} / ${renderer?.migrationMissingLayerGroupCount ?? runtime?.migrationMissingLayerGroups.length ?? pixGridState.configuration.lastMigration?.missingLayerGroups?.length ?? 0}`],
            ['Ineffective routes / effective live', `${renderer?.migrationIneffectiveAssignmentCount ?? runtime?.migrationIneffectiveAssignments.length ?? pixGridState.configuration.lastMigration?.ineffectiveAssignments?.length ?? 0} / ${renderer?.migrationEffectiveLiveRouteCount ?? runtime?.migrationEffectiveLiveRouteCount ?? pixGridState.configuration.lastMigration?.effectiveLiveRouteCount ?? 0}`],
            ['Safe recovery', (renderer?.migrationSafeRecoveryUsed ?? runtime?.migrationSafeRecoveryUsed ?? pixGridState.configuration.lastMigration?.safeRecoveryUsed) ? 'Used' : 'Not used'],
