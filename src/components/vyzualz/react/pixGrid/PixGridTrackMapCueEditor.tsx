...
function TargetField({ value, state, onChange }: TargetFieldProps) {
  return (
    <label className="rv-pix-grid-cue-field">
      <span>Target</span>
      <select value={targetValue(value)} onChange={event => onChange(parseTarget(event.target.value))}>
...
function ActionFields({ action, state, onChange }: {
  ...
  switch (action.type) {
    case 'selectScene':
      return <label className="rv-pix-grid-cue-field"><span>Scene</span><select value={action.sceneId} onChange={event => onChange({ ...action, sceneId: event.target.value })}>{state.scenes.map(scene => <option key={scene.id} value={scene.id}>{scene.name}</option>)}</select></label>
    case 'setLayerVisible':
      return <><label className="rv-pix-grid-cue-field"><span>Layer</span><select value={action.layerId} onChange={event => onChange({ ...action, layerId: event.target.value })}>{layerOptions.map(layer => <option key={layer.id} value={layer.id}>{layer.name}</option>)}</select></label><label className="rv-pix-grid-cue-check"><input type="checkbox" checked={action.visible} onChange={event => onChange({ ...action, visible: event.target.checked })} /> Visible</label></>
    case 'setGroupVisible':
      return <><label className="rv-pix-grid-cue-field"><span>Group</span><select value={action.groupId} onChange={event => onChange({ ...action, groupId: event.target.value })}>{groupOptions.map(group => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label><label className="rv-pix-grid-cue-check"><input type="checkbox" checked={action.visible} onChange={event => onChange({ ...action, visible: event.target.checked })} /> Visible</label></>
    case 'flashGroup':
      return <><label className="rv-pix-grid-cue-field"><span>Group</span><select value={action.groupId} onChange={event => onChange({ ...action, groupId: event.target.value })}>{groupOptions.map(group => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label><label className="rv-pix-grid-cue-field"><span>Amount</span><input type="number" min="0" max="2" step="0.05" value={action.amount} onChange={event => onChange({ ...action, amount: numeric(event.target.value, 1) })} /></label></>
    case 'revealRows':
      return <><TargetField value={action.target} state={state} onChange={target => onChange({ ...action, target })} /><label className="rv-pix-grid-cue-field"><span>Origin</span><select value={action.from} onChange={event => onChange({ ...action, from: event.target.value as typeof action.from })}><option value="top">Top</option><option value="bottom">Bottom</option><option value="center">Center</option></select></label></>
    case 'revealColumns':
    ...
    case 'dissolveGroup':
      return <><label className="rv-pix-grid-cue-field"><span>Group</span><select value={action.groupId} onChange={event => onChange({ ...action, groupId: event.target.value })}>{groupOptions.map(group => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label><label className="rv-pix-grid-cue-field"><span>Amount</span><input type="number" min="0" max="1" step="0.05" value={action.amount} onChange={event => onChange({ ...action, amount: numeric(event.target.value, 1) })} /></label></>
    case 'setPaletteMode':
      return <label className="rv-pix-grid-cue-field"><span>Palette</span><select value={action.mode} onChange={event => onChange({ ...action, mode: event.target.value as typeof action.mode })}><option value="original">Original</option><option value="hybrid">Hybrid</option><option value="brand">Brand Kit</option><option value="preset">Preset</option></select></label>
    case 'setBackground':
      return <><label className="rv-pix-grid-cue-field"><span>Mode</span><select value={action.mode} onChange={event => onChange({ ...action, mode: event.target.value as typeof action.mode })}><option value="preset">Preset</option><option value="black">Black</option><option value="custom">Custom</option></select></label>{action.mode === 'custom' && <label className="rv-pix-grid-cue-field"><span>Color</span><input type="color" value={action.color ?? '#000000'} onChange={event => onChange({ ...action, color: event.target.value })} /></label>}<label className="rv-pix-grid-cue-field"><span>Brightness</span><input type="number" min="0" max="1" step="0.05" value={action.brightness ?? 0} onChange={event => onChange({ ...action, brightness: numeric(event.target.value, 0) })} /></label></>
    case 'startAnimation':
      return <><TargetField value={action.target} state={state} onChange={target => onChange({ ...action, target })} /><label className="rv-pix-grid-cue-field"><span>Animation</span><select value={action.animation} onChange={event => onChange({ ...action, animation: event.target.value as typeof action.animation })}>{['pulse','bounce','horizontalScroll','verticalScroll','pingPong','rotate','paletteCycle','blink','revealRow','revealColumn','checkerAlternate','frameCycle','audioAmplitudeScale','beatStepMovement'].map(mode => <option key={mode} value={mode}>{mode.replace(/([A-Z])/g, ' $1')}</option>)}</select></label><label className="rv-pix-grid-cue-field"><span>Clock</span><select value={action.clock} onChange={event => onChange({ ...action, clock: event.target.value as typeof action.clock })}><option value="time">Time</option><option value="beat">Beat</option><option value="bar">Bar</option><option value="cue">Cue</option></select></label><label className="rv-pix-grid-cue-field"><span>Speed</span><input type="number" min="-20" max="20" step="0.05" value={action.speed} onChange={event => onChange({ ...action, speed: numeric(event.target.value, 1) })} /></label><label className="rv-pix-grid-cue-field"><span>Amount</span><input type="number" min="-4" max="4" step="0.05" value={action.amount} onChange={event => onChange({ ...action, amount: numeric(event.target.value, 0.25) })} /></label></>
    case 'stopAnimation':
  ...
  return (
    <div className="rv-pix-grid-cue-editor" role="dialog" aria-label={`${isNew ? 'Create' : 'Edit'} PixGrid action cue`}>
      ...
      <div className="rv-pix-grid-cue-grid">
        ...
        <label className="rv-pix-grid-cue-field"><span>Action</span><select value={draft.action.type} onChange={event => setAction(defaultPixGridCueAction(event.target.value as PixGridActionCueAction['type']))}>{ACTION_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
        <label className="rv-pix-grid-cue-field"><span>Quantize</span><select value={draft.quantization} onChange={event => setDraft(current => ({ ...current, quantization: event.target.value as PixGridActionCue['quantization'] }))}><option value="none">Free</option><option value="beat">Beat</option><option value="bar">Bar</option><option value="fourBars">4 bars</option></select></label>
        <label className="rv-pix-grid-cue-field"><span>Transition</span><select value={draft.transition} onChange={event => setDraft(current => ({ ...current, transition: event.target.value as PixGridCueTransition }))}>{TRANSITIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
        ...
        <label className="rv-pix-grid-cue-field"><span>Loop</span><select value={draft.loopBehavior} onChange={event => setDraft(current => ({ ...current, loopBehavior: event.target.value as PixGridActionCue['loopBehavior'] }))}><option value="retrigger">Retrigger</option><option value="once">Once per load</option></select></label>
