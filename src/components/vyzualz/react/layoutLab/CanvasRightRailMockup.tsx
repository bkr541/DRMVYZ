...
/** Concept — "Route Tree." A round number badge leads the effect input; an
 * L-shaped connector trunk drops from it and branches to each routed
 * parameter card. The badge toggles the shared editor. Local route state. */
function AddEffectsRouteTreeConcept({ state }: { state: CanvasMockState }) {
  const { links, routesFor, isOpenFor, toggle, editorHandlers } = useConceptRoutes()
  return (
    <ConceptGroup
      state={state}
      label="Add Effects — Route Tree"
      note="A round number badge leads the effect input; an L-shaped connector trunk drops from it and branches to each routed parameter card. The badge toggles the parameter editor. Concept only."
    >
      {(layer, layerIndex) => (
        <AddEffectsLayerGroup
          key={layer.mediaId}
          ...
          getEntryExtra={({ linkKey }) => {
            const color = firstRouteColor(links[linkKey])
            return {
              className: `rv-ae-tree-entry${color ? ' is-linked' : ''}`,
              style: color ? ({ '--tree-color': color } as CSSProperties) : undefined,
            }
          }}
          renderLeading={({ effectIndex, effectLabel, parentLabel, linkKey }) => {
            const routes = routesFor(linkKey)
            return (
              <button
                type="button"
                className={`rv-ae-tree-badge${routes.length ? ' is-linked' : ''}`}
                aria-expanded={isOpenFor(linkKey)}
                aria-label={`${routes.length ? 'Edit' : 'Add'} routes for ${effectLabel} on ${parentLabel}`}
                onClick={() => toggle(linkKey)}
              >
                {effectIndex + 1}
          ...
          renderRoute={({ effectLabel, parentLabel, linkKey }) => {
            if (!isOpenFor(linkKey)) return null
            return (
              <div className="rv-ae-tree">
                <AddEffectsRouteEditor routes={routesFor(linkKey)} effectLabel={effectLabel} parentLabel={parentLabel} showDots {...editorHandlers(linkKey)} />
              </div>
            )
          }}
        />
...
/** Concept — "Signal Ladder." Each effect is a color-bordered card hung off a
 * numbered node on a vertical spine; a route-count subtitle and a "+ Add
 * Route" action sit inside it. Local route state, comparison only. */
function AddEffectsSignalLadderConcept({ state }: { state: CanvasMockState }) {
  const { links, routesFor, isOpenFor, toggle, editorHandlers } = useConceptRoutes()
  return (
    <ConceptGroup
      state={state}
      label="Add Effects — Signal Ladder"
      note="Each effect is a color-bordered card hung off a numbered node on a vertical spine. A subtitle counts its routes; “+ Add Route” opens the parameter list. Concept only."
    >
      {(layer, layerIndex) => (
        <AddEffectsLayerGroup
          key={layer.mediaId}
          ...
          getEntryExtra={({ effectIndex, linkKey }) => {
            ...
            const color = routed ?? RACK_FALLBACK_COLORS[effectIndex % RACK_FALLBACK_COLORS.length]
            return {
              className: `rv-ae-ladder-entry${routed ? ' is-linked' : ''}`,
              style: { '--ladder-color': color } as CSSProperties,
            }
          }}
          renderLeading={({ effectIndex }) => (
            <span className="rv-ae-ladder-node" aria-hidden="true">{effectIndex + 1}</span>
          )}
          renderRoute={({ effectLabel, parentLabel, linkKey }) => {
            const routes = routesFor(linkKey)
            const open = isOpenFor(linkKey)
            return (
              <>
                <div className="rv-ae-ladder-sub">
                  {routes.length} ROUTE{routes.length === 1 ? '' : 'S'} · AUDIO INTELLIGENCE
                </div>
                {open && (
                  <div className="rv-ae-ladder-body">
                    <AddEffectsRouteEditor routes={routes} effectLabel={effectLabel} parentLabel={parentLabel} showDots {...editorHandlers(linkKey)} />
                  </div>
                )}
                <button
                  type="button"
                  className={`rv-ae-ladder-add${routes.length ? ' is-linked' : ''}`}
                  aria-expanded={open}
                  aria-label={`${open ? 'Hide' : 'Show'} routes for ${effectLabel} on ${parentLabel}`}
                  onClick={() => toggle(linkKey)}
                >
                  + Add Route
                </button>
...
/** Concept — "Patch Bay." The effect input reads as a rounded pill node with
 * an "Effect NN" caption; a ring-node connector spine branches down to each
 * parameter card. Local route state, comparison only. */
function AddEffectsPatchBayConcept({ state }: { state: CanvasMockState }) {
  const { links, routesFor, isOpenFor, toggle, editorHandlers } = useConceptRoutes()
  return (
    <ConceptGroup
      state={state}
      label="Add Effects — Patch Bay"
      note="The effect input reads as a rounded pill node with an “Effect NN” caption; a connector spine with ring nodes branches down to each parameter card, its intensity shown in a boxed value. Concept only."
    >
      {(layer, layerIndex) => (
        <AddEffectsLayerGroup
          key={layer.mediaId}
          ...
          getEntryExtra={({ linkKey }) => {
            const color = firstRouteColor(links[linkKey])
            return {
              className: `rv-ae-patch-entry${color ? ' is-linked' : ''}`,
              style: color ? ({ '--patch-color': color } as CSSProperties) : undefined,
            }
          }}
          renderLeading={({ effectIndex, effectLabel, parentLabel, linkKey }) => {
            const routes = routesFor(linkKey)
            return (
              <button
                type="button"
                className={`rv-ae-patch-badge${routes.length ? ' is-linked' : ''}`}
                aria-expanded={isOpenFor(linkKey)}
                aria-label={`${routes.length ? 'Edit' : 'Add'} routes for ${effectLabel} on ${parentLabel}`}
                onClick={() => toggle(linkKey)}
              >
          ...
          renderRoute={({ effectIndex, effectLabel, parentLabel, linkKey }) => {
            const caption = <div className="rv-ae-patch-caption">Effect {String(effectIndex + 1).padStart(2, '0')}</div>
            if (!isOpenFor(linkKey)) return caption
            return (
              <>
                {caption}
                <div className="rv-ae-patch-editor">
                  <AddEffectsRouteEditor routes={routesFor(linkKey)} effectLabel={effectLabel} parentLabel={parentLabel} showDots {...editorHandlers(linkKey)} />
                </div>
              </>
            )
    ...
    <WorkspaceBody>
      ...
      {state.reactSurface === 'routing' ? (
        <div className="rv-ctrl-group" data-layout-lab-canvas="routing">
          ...
          <AddEffectsRouteTreeConcept state={state} />
          <AddEffectsSignalLadderConcept state={state} />
          <AddEffectsPatchBayConcept state={state} />
        </div>
      ) : <AnalysisMockup />}
    </WorkspaceBody>
  )
