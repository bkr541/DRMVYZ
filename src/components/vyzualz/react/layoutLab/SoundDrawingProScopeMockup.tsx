...
// ── SoundDrawingProScopeMockup ────────────────────────────────────────────────
//
// A disconnected copy of soundDrawing/SoundDrawingProScopeControls.tsx for
// Layout Lab. Same fields, same layout, same collapsible groups — driven by a
// local osc/set pair instead of the real oscillator store. The only thing the
// real component reads from the live engine (engine.scopeStereoTap capture
// status, for the two advisory notes at the top) is fixed to "healthy" here
// since there is no audio tap to ask.
...
export function SoundDrawingProScopeMockup({ osc, set, hideTraceSize = false }: Props) {
