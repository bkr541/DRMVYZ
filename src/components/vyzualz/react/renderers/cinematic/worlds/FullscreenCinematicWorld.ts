...
export abstract class FullscreenCinematicWorld implements CinematicWebGLWorldRenderer {
  ...
  render(frame: CinematicFrameContext, target: CinematicWorldRenderTarget): void {
    ...
    // A fully self-driven world (e.g. Afterhours) ignores the shared time /
    // audio / palette / camera uniforms and pushes its own, so skip the whole
    // block — it is pure wasted work for that world's shader.
    const shared = this.consumesSharedFrameUniforms()
  ...
  /**
   * Whether the shared per-frame time / audio / palette / camera uniform block
   * is written. Default `true`. A world whose shader consumes none of them
   * overrides this to `false` to skip the work.
   */
  protected consumesSharedFrameUniforms(): boolean {
    return true
  }
