...
const output = path.join(root, 'artifacts/pix-grid-deck-show-manager-browser')
...
try {
  ...
  await build({
    ...
    build: {
      ...
      emptyOutDir: true,
      rollupOptions: {
        input: path.join(root, 'src/test/browser/pix-grid-deck-show-manager.html'),
      },
    },
  })
  const builtHtml = await findFile(output, 'pix-grid-deck-show-manager.html')
  if (!builtHtml) throw new Error('Vite did not emit the PixGrid Deck Show Manager browser page.')
  const pagePath = `/${path.relative(output, builtHtml).split(path.sep).join('/')}`
  const env = {
    ...process.env,
    DRMVYZ_PIX_GRID_DECK_SHOW_MANAGER_BROWSER: '1',
    DRMVYZ_PIX_GRID_DECK_SHOW_MANAGER_PAGE: pagePath,
  ...
  const result = spawnSync(process.execPath, [
    playwrightCli,
    'test',
    'src/test/e2e/pixGridDeckShowManager.spec.ts',
    '--project=chromium',
  ], { cwd: root, env, stdio: 'inherit' })
...
console.log('PixGrid Deck Show Manager acceptance passed.')
