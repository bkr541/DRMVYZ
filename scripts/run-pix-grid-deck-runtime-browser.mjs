...
const output = path.join(root, 'artifacts/pix-grid-deck-runtime-browser')
...
try {
  ...
  await build({
    ...
    build: {
      ...
      emptyOutDir: true,
      rollupOptions: {
        input: path.join(root, 'src/test/browser/pix-grid-deck-runtime.html'),
      },
    },
  })
  const builtHtml = await findFile(output, 'pix-grid-deck-runtime.html')
  if (!builtHtml) throw new Error('Vite did not emit the PixGrid Deck runtime browser page.')
  const pagePath = `/${path.relative(output, builtHtml).split(path.sep).join('/')}`
  const env = {
    ...process.env,
    DRMVYZ_PIX_GRID_DECK_RUNTIME_BROWSER: '1',
    DRMVYZ_PIX_GRID_DECK_RUNTIME_PAGE: pagePath,
  ...
  const result = spawnSync(process.execPath, [
    playwrightCli,
    'test',
    'src/test/e2e/pixGridDeckRuntimeRendering.spec.ts',
    '--project=chromium',
  ], { cwd: root, env, stdio: 'inherit' })
...
console.log('PixGrid Deck real runtime acceptance passed.')
