...
missing, invalid or shared files; a file over the per-file budget; a texture over the size limit (volumes: edge over 128 texels, or an image that is not a whole number of square slices); a model over the triangle limit; an asset whose estimated GPU
