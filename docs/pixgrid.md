...
Deck ingestion is an optimistic semantic transaction. Validation, upload, and compiler preflight may run while the user reorders, enables, disables, removes, renames, or reconfigures the target Deck. Immediately before commit, the service re-reads the current project generation and current Deck, rejects a replaced project or deleted Deck, removes duplicate or over-capacity candidates, rebases accepted items after the latest ordered item list, and performs one synchronous store mutation. Cancellation on Deck Builder exit or unmount rolls back unowned uploaded media and ignores late UI callbacks.

`PixGridDeckProjectPortability.ts` is the production adapter boundary for Deck source ownership. The supplied application currently has no general user-facing project archive service or archive registry beyond this adapter, so no parallel archive format is introduced. A future shared project exporter must call `exportCurrentPixGridDeckProjectMediaBundle`; its importer must call `importPixGridDeckProjectMediaBundleIntoStore` after archive validation. The bundle manifest contains stable Deck definitions, source fingerprints, storage metadata, and source files, but never signed URLs, local object URLs, or compiled output.

The standalone Show Manager browser harness remains deterministic architecture coverage. Authentication-aware production coverage is separate and cannot pass at the login screen. Create a Playwright storage-state JSON through the normal Supabase login flow on the same preview origin used by the test, keep it outside source control, export `DRMVYZ_E2E_AUTH_STORAGE_STATE`, `VITE_SUPABASE_URL`, and `VITE_SUPABASE_ANON_KEY`, then run `npm run test:e2e:pix-grid-deck-authenticated`. The runner validates that the state contains a Supabase session, performs a production build, enters the real `App` authentication gate, uploads through the canonical media service, waits for the authenticated Deck compiler runtime, creates the generated Preset, and verifies that the PixGrid surface consumes it through WebGL or Canvas2D. Production authentication code is unchanged.
