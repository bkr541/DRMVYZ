...
(#5, #6, #7 and the native half of #10 delivered; #8, #9 and the Three half of #10 open) · Parked ideas and the next test scene (blossom chamber, ten instancing ideas, logo/SVG findings) · Threshold refinement plan and batches A and B · Practical recipes and gotchas · Open decisions for the owner.
...
| 7 follow-up | Smoke volume for the haze | DONE 2026-09-25 (see "Smoke volume delivered"). Texture service gained 3D volumes; shipped tileable `cinema2-smoke-volume` (64^3, 32^3 on low); `volumetric-atmosphere` gained `smokeTexture`/`smokeContrast`/`smokeWarp`/`smokeStrength` (domain-warped two-sample lookup replacing the procedural fbm); Threshold uses it: billowing, curling smoke instead of smooth noise haze, and it is CHEAPER (high 7.2 ms vs 8.2 ms procedural) |
| 9 (native half) | Threshold detail, budgets, culling and ground smoke | DONE 2026-09-25 (see "#9 native half delivered"). An outer rank of taller towers (an overhead structure of beams, rails and cables was built first and REMOVED at the owner's request: the upper part of the frame stays open) gated by quality tier, deterministic seeded layout, per-tier instance budgets checked at module creation, chunked lap culling, and ground-smoke billboards (shipped `cinema2-smoke-sprites`) lit by the screens. +0.4-0.5 ms high, +0.3 ms medium, ~0 low (1080p) |
| 8 | PBR, environment lighting and floor glare | DONE 2026-09-25 (see "#8 delivered"). `three-scene`: clearcoat (physical materials), shipped HDR studio environment with rotation/exposure-tied intensity and a fail-over to the built-in room, rectangular LED-panel lights (`RectAreaLight`) by tier; native `reflective-floor`: screen-row glare so Threshold's screens light the wet floor. Cost: whole reference scene high 11 ms / medium 6-7 / low 3 at 1080p; Threshold glare +0.2 ms high |
| fix | Threshold audio reaction, BPM Sync and Camera Motion (owner bug report 2026-09-25) | DONE (see "Threshold owner-reported bugs fixed"). Root causes found and fixed: half of every lap had no lit screens (ring/field faces and gating), audio mapping could leave the set dim, a drop was only a one-second flash, BPM Sync did almost nothing (and was silently gated by the dock SYNC button), Camera Motion amplitudes were invisible |
| HUM:N | HUM:N rebuilt as a real 3D low-poly figure (owner request 2026-09-25) | DONE (see "HUM:N 3D figure rebuild"). Skinned 3D mesh + bone rig, per-triangle fills/gradients/stripes/dots, one Master Intensity, working BPM Sync, Auto Color from the music, Auto Performance owns every pose, no grid, Figure Scale to 2.5x, Flicker/Jitter visibly react. 1.6-1.8 ms at 1080p |
Test state: `npx vitest run src/components/vyzualz/cinema2` = the same 15 failing files / 40 failing tests that pre-date all this work (listed under "What exists today"), 641 passing after the HUM:N rebuild (648 before it; the deleted 2D suites and the new 3D ones differ in size) (633 after #8); `npm run test:assets` 16 passing; lint and typecheck clean for touched files.

### Remaining issues and limits (known, not fixed)
- Threshold vs the reference: the haze is now billowing, wispy smoke (shipped smoke volume) but it is still a density modulation, not simulated or accumulated smoke and there are no sprites at the tower bases (#9); the towers now block the key light (shafts read as shaded haze; still no hard diagonal beams, see "#10 delivered"); floor now uses the shipped wet-concrete map (cracks, mottling, broken reflections; the water look is largely gone) but has the screens now light the floor near the towers (analytic glare, all rows share one intensity) but there is no per-row flash or puddle glare and the texture is a single generated tile; the screens do not light the floor or air (only the towers); housings are boxes without bolts/bevels (the overhead structure now exists); the corridor still has 7 pairs (depth now comes from an outer tower rank, not more pairs, because the hanging field starts where a longer corridor would end); the hanging field and ring were not touched; low tier is dearer than before batch B (3.7 ms vs 1.9 ms, cause not investigated).
- `three-scene`: no Draco (meshopt or uncompressed only); one set of material overrides per module (no per-instance overrides); panel lights do not cast shadows and cost per light (0/2/4 by tier); the native reflective floor still reflects a flat sky colour where screen-space misses (the Three environment is not shared with it); no skinned/animated meshes; no MSAA; layer `depthPolicy` is not passed to world providers (Three always tests and writes depth); quality variant chosen only when loading starts (a later low->high switch changes materials only and recompiles shaders once); environment is the procedural RoomEnvironment; no visible preset uses it yet.
...
| #8 PBR + environment lighting (DONE except the native floor's environment reflections and Three-side shadows from panels) | Physical materials, shipped HDR environment, area lights for emissive panels | Floor glare/pools from the screens (could also be approximated natively), real environment reflections for screen-space misses, glossy skull/alien/wet rock; LESS important for Threshold's own housings (already lit analytically) | Area lights need Standard/Physical materials, no shadows from area lights; cost scales with light count |
| #9 instancing + distance detail (native half DONE for Threshold; Three InstancedMesh/LOD half and a generic instanced-mesh module still open) | Generalise Threshold's instanced box renderer; Three `InstancedMesh`, LODs, impostors | More receding towers, overhead truss/cable detail, smoke drawn as textured billboards at the tower bases (with #7a), foliage/rocks for the jungle/cave | Native half needs no Three; per-tier instance budgets and deterministic seeded scatter |
...
- Existing presets (Interlock, Afterhours, Electric Storm, Reactor…) are not migrated (HUM:N WAS rebuilt as a real 3D figure on 2026-09-25, see "HUM:N 3D figure rebuild"). Nothing new is loaded or paid
...
- Presets (`presets/Cinema2FirstPartyPresetCatalog.ts`): foundation, reference visual, Reactor, Spatial Reference (internal), Atmosphere Reference (visible),
  Electric Storm, Afterhours, Interlock, HUM:N (now a 3D figure), **Threshold**. Presets tagged `internal` are hidden from the Cinema 2.0 preset list.
...
### Smoke volume delivered (7a follow-up, 2026-09-25)
- Texture service: `noise-volume-rgba` layout uploads one tall image (`width` wide, `width * depth` tall, square slices stacked top to bottom) as a 3D texture (`texImage3D`, RGBA8, mipmaps, REPEAT on all axes). Handles expose `dimension` ('2d' | '3d') and `depth`; budgeting prices volumes by w*h*depth; a ragged image (not whole square slices) fails with a diagnostic. `FullscreenPass` texture bindings gained an optional `target` (`gl.TEXTURE_3D`) so effects can bind volume samplers; default behaviour unchanged.
- Asset: `cinema2-smoke-volume`, generated in house by `scripts/cinema2-assets/generate-smoke-volume.mjs` (periodic Perlin/Worley noise, continuous in position so the 32^3 low file is the same field sampled more coarsely). Channels: R billowy puffs (Perlin-Worley), G ridged wisps, B fine puffs, A broad patchiness. Lossless WebP: 64^3 = 660 KB (1.4 MB GPU), 32^3 = 104 KB (0.17 MB GPU). The pipeline (`assets-core.mjs`) understands the layout (depth from the image shape, volume edge limit 128, GPU price by volume); `assets/cinema2/README.md` documents it.
- `volumetric-atmosphere`: `smokeTexture` (asset id, layout checked at validate), `smokeContrast` (0.5-4, default 1.6), `smokeWarp` (0-2, default 0.6), `smokeStrength` (0-1, default 1). In the march, `noiseAt(p)` samples the volume twice with explicit LOD (coarse billows warp the lookup of a finer wispy layer) and blends with the procedural fbm by the fade amount; it fades in over 0.8 s once loaded, follows the quality variant, and a missing/failed volume keeps the procedural noise (Runtime status: "an effect skips a texture it could not load"). The effect owns a 1x1x1 fallback 3D texture so the sampler is always valid. Texture unit 5.
- Threshold authors `smokeTexture: cinema2-smoke-volume`, contrast 1.6, warp 0.6.
Measured in real Chrome, M3 Pro, 1080p whole Threshold chain (tight loop after warm-up): high 7.2 ms with smoke vs 8.2 ms with the procedural noise (two texture fetches are cheaper than three octaves of hashed value noise), medium 4.6 both, low 2.1 both. A/B frames: smoke off = smooth cloud, smoke on = billowing, curling wisps with real contrast. 404 on the volume: procedural haze, status message shown, no errors. Context loss: textures drop to 0 bytes and both re-upload after restore. Tests: 12 asset-pipeline tests (volume shape/price/limits), 15 texture-asset tests (3D upload, ragged rejection, variant depth), volumetric smoke validation + fade/quality/dispose test. Cinema2 folder: same 15 failing files / 40 tests as before, 608 passing; `react/shaders` failures (2 files / 5 tests) are identical on clean HEAD.
Not done: the smoke is a density modulation of the existing haze, not billboard sprites or accumulated simulated smoke (that is #9); the noise is static in shape, drifting only with the existing `drift` wind; one volume shipped.

### #9 native half delivered (Threshold, 2026-09-25)
Scope decision: the generic "instanced mesh module" was NOT extracted (there is only one consumer, Threshold, and its box renderer already is the instanced primitive renderer); instead the parts the spec asks for were added to it and are reusable: per-tier gating, budgets, seeded scatter, chunked culling, billboards.
- Layout (`modules/threshold/Cinema2ThresholdLayout.ts`): `buildThresholdLayout(seed, { extras })` appends `buildThresholdDetail(seed)` after the unchanged base layout (own seeded PRNG stream, so the base scatter and indices never change): an outer rank of taller dark towers staggered behind the housings (medium and up). All role 0, on the floor outside the housings (asserted). (The overhead structure of cross beams, rails, chords, struts and cables that was first built here came from my own reading of the reference's 'faint overhead structure' - difference #10 - not from an owner requirement; the owner asked for it to be removed and it was, with the outer towers and the smoke kept.) Instances carry `minTier` (packed in the reserved i3.z float); the vertex shader moves anything above the current tier off screen (main and shadow passes).
- Budgets: `THRESHOLD_INSTANCE_BUDGETS` per tier (boxes / puffs per lap: low 200/30, medium 320/60, high 420/120); the module checks its layout against all three at creation and throws (module fails, engine keeps running) if detail would exceed a tier. Counts today: boxes 160 base + 16 outer towers (the removed overhead structure had added 82 more); puffs 28 / 42 / 56.
- Chunk culling: `buildThresholdChunks` (24 consecutive instances with a conservative lap-local z extent) and `visibleThresholdRanges` (merged draw ranges for a lap copy given camera z, view distance and a margin). The renderer draws laps far to near and, per lap, only visible ranges (re-pointing the instance attributes at the range start): the lap behind the camera is now skipped entirely. The shadow pass uses the same culling with a 110-unit margin behind the camera (the light frustum reaches behind it).
- Ground smoke: `buildThresholdSmoke(seed)` puffs at the tower bases (2 per side per pair on all tiers, +1 medium, +2 centre puffs high), sorted farthest first so the instance buffer draws back to front. Camera-facing billboards (`SMOKE_VERTEX_SOURCE`) sampling the shipped 2x2 sprite sheet `cinema2-smoke-sprites` (new `sprite-sheet-rgba` layout, 512 px / 256 px low, 121 KB / 41 KB): rotating and drifting, breathing with bass, lit by the screen row in front of them via the SAME emission function the towers use (`towerEmission`, extracted into shared GLSL so they cannot drift apart), thinned near the housing walls and the camera, faded with distance, and brightened with distance to make up for the haze the depth-aware haze pass cannot accumulate behind them; far ones are backlit by the vanishing glow. Two draws per lap: a blended colour pass without depth write, then a depth-only pass for the dense core (alpha > 0.7) so the reflective floor occludes smoke below it and reflects the core while thin smoke does not cut holes in the haze. Modules got `textures` in their create context (`Cinema2ModuleCreateContext.textures`, passed by the runtime); the sprite handle follows the quality variant, fades in over 0.8 s and never blocks the scene.
- Config: Threshold module `config.extras` (default true; false = the pre-#9 scene exactly, used for A/B). No new user controls.
Measured in real Chrome, M3 Pro, 1080p, tight loop after warm-up: high 7.7-7.8 ms with detail vs 7.3 ms without, medium 4.8 vs 4.5, low 2.2 both. With extras off the frames match the pre-change baseline to within float noise (0.1% of pixels differ by a few LSB, run-to-run deterministic). Visual: smoke pools at the tower bases and billows down the aisle. Context loss: the module stays active and the sprite sheet re-uploads. Tests: layout determinism/tier gating/budgets/lane clearance, smoke ordering and packing, chunk coverage and lap culling, module config validation, per-preset asset budgets now include smokeTexture and Threshold's sprite sheet, pipeline sprite-sheet test. Cinema2 folder: same 15 failing files / 40 tests as before, 616 passing.
Look-tuning lessons: (1) an alpha-blended billboard that writes depth cuts holes in the depth-aware haze and reads as a dark silhouette, so depth is written only for the dense core; (2) the floor effect overwrites every pixel where the floor plane is nearer than the scene depth, which erases depth-less smoke below the horizon, hence the depth pass; (3) at the kick frames the towers' emission is very high, so smoke emission is capped at 1.5.
Not done: the corridor length/pair count (needs a layout period change), Three `InstancedMesh`/LOD/impostor half, wind-swayed foliage, screen-space soft-particle fade (smoke fades analytically near walls instead), one shipped sprite sheet, smoke does not respond to the key light or receive shadows, per-frame CPU culling is z-only (no lateral frustum test).
...
### #8 delivered (2026-09-25)
Scope: everything the spec lists for the Three side plus the native floor glare for Threshold; the two optional/derived items noted as not done at the end.
- Pipeline: new asset `kind: 'environment'` (equirectangular 2:1 Radiance `.hdr`, license/origin rules as before, width limit 2048, GPU priced as the filtered cube map on every tier) with `readHdrSize`; registry `modules/three/Cinema2ThreeEnvironmentRegistry.ts` filled from the generated manifest. Shipped `cinema2-studio-environment`: generated in house by `scripts/cinema2-assets/generate-studio-environment.mjs` (dark room, overhead panel, bright key softbox, cool strip, magenta accent; real HDR, peaks 10-20; standard Radiance RLE) - 1024x512 = 149 KB, 512x256 = 41 KB for low.
- Three library/host: `HDRLoader` (the non-deprecated loader in three 0.186) loaded with the library; the 100 KB-gzip `RectAreaLightUniformsLib` chunk loads only for modules that configure panels (`library.loadAreaLightTables()`; a failed load turns panels off with a diagnostic). Host `loadEnvironment(url)`: fetch, checks the `#?RADIANCE` magic (a dev server/app protocol answers unknown paths with an HTML page and status 200), decodes, PMREM once per URL per context (cached, dropped on context loss, failed loads not cached).
- `three-scene` module: new parameters `clearcoat`, `clearcoatRoughness`, `environmentRotation` (degrees), `panelIntensity`; new config `environment` (shipped id) and `panels: [{ position, target, size, color, intensity }]`, both validated at compile time. Clearcoat upgrades materials to `MeshPhysicalMaterial` on first use (copying the asset's own values), and is held at >= 0.001 on the tiers that draw it so turning it up never recompiles a shader (0 on low). Environment intensity is multiplied by the Cinema 2.0 environment exposure; rotation turns `scene.environmentRotation`. Panels are a fixed pool of `RectAreaLight`s (visible: 0 low, 2 medium, 4 high) whose intensity is `config intensity * panelIntensity`. The scene draws nothing until the shipped environment is ready; if it cannot load, the built-in studio room is used and a `CINEMA2_THREE_ENVIRONMENT_LOAD_FAILED` diagnostic makes the Runtime status say a module skipped an asset.
- Three Model Reference preset: shipped environment + two panels + new controls (Clearcoat, Environment Rotation, Panel Light).
- Native floor glare (`reflective-floor`): `glareIntensity` (default 0 = off), `glareColor`, `glareOffsetX` x `glareWidthScale`, `glareFirstDistance`, `glareSpacing`, `glareRowCount`, `glareRepeat`, `glareScreenHeight`, `glareArea`: area-light style falloff from the three nearest rows of tall screens on each side, repeating every lap. Threshold authors it from the new exported `THRESHOLD_CORRIDOR` layout constants, binds intensity to Master Intensity and width to Corridor Width, and pulses it with downbeat/kick/drop envelopes like the screens. (Threshold's screens do not pass per-row flashes to the floor: all rows share one glare.)
Measured in real Chrome, M3 Pro, 1080p, tight loop: Three Model Reference whole frame high 11 ms, medium 6-7 ms, low 3 ms (the #6 measurement of the same scene was 9.1 / 5 / 2.5 ms in an earlier session, so PBR costs roughly +1-2 ms; the two panel lights are inside the noise: 11.0 without panels, 11.4 with two); Threshold with glare high 8.0 ms vs 7.8 off, medium 4.7 vs 4.6. Acceptance: environment rotation 0 vs 90 / 180 changes the knots strongly (mean abs difference 21 / 18 of 255 in the model region), panel intensity 0 vs 6 changes them (15.5), clearcoat 0 vs 1 (3.7), a light colour change (3.1), a missing/HTML environment falls back with the diagnostic and looks different (18.7); low tier has no clearcoat and no panels but keeps image-based lighting; context loss recovers with the module active. Production build: HDRLoader and the area-light tables are separate lazy chunks, the `.hdr` files ship in `dist/cinema2/environments/` and are served by the real Electron `drmvyz-app://` protocol (content type `application/octet-stream`, bytes intact, `#?RADIANCE` header). Tests: `Cinema2ThreePbr.test.ts` (12: registry, validation, panel pool by tier, physical upgrade and clearcoat floor, rotation/exposure, environment wait and fail-over, dispose; real Three objects with a stand-in renderer host), module test for the lazy area-light tables, pipeline tests for environments (16 total), floor glare tests, environment ids in the per-preset asset budget. Cinema2 folder: same 15 failing files / 40 tests as before, 633 passing.
Not done: the native reflective floor still reflects a flat sky colour where screen-space misses (the shipped environment is a Three PMREM cube; sharing it with the native effect would need a second texture form of it); panel lights cast no shadows (Three limitation) and are not fed from the Cinema 2.0 light list (they are configured on the module, because the engine has no rectangular light type); a preset that wants the shipped environment on Three models must opt in with `config.environment`; only one environment shipped; per-row screen flashes are not reflected in the floor glare.
...
### Threshold owner-reported bugs fixed (2026-09-25)
The owner reported four problems in Threshold: (1) BPM Sync seemed to do nothing when toggled with tracks of different tempo, and the screens must light up and react to the track's audio intelligence, (2) the Camera Motion slider did nothing visible, (3) whole sections with no lit LED screens, just forward movement, (4) at a track's DROP section (screenshot: huge dark slabs, nothing lit) the visual did nothing significant. All four were reproduced in real Chrome with realistic music frames before fixing (`screens(>235)` lit-pixel fraction and camera pose logged per frame), and re-measured after.
Root causes:
- (3) and (4): the flight spends about half of every 96 s lap (from z = -106) in the hanging field and the ring. There, the LED faces looked away or edge-on: field slabs were turned up to 48 degrees, and the ring's near-side panels face the ring centre, so the camera approaches their BACKS (a giant black slab dead ahead, exactly the screenshot). Measured lit-screen fraction from t = 50 to 70 s was 0-3% even with no audio at all. With music, the audio mapping made it worse: screens 'opened' by random rank against an arc that fell to 0.3 at low energy, and the level dimmed to 0.55.
- (4) also: a drop only produced a one-shot flash (rising edge of the drop confidence, about 1.1 s), and the sections (drop, build, breakdown) were never used. In a sustained loud drop `energy` is moderate and `buildProgress` is about 0 (it measures rising energy), so nothing stood out.
- (1): the preset toggle only scaled a clock speed by bpm/120 (breathing, shimmer, sweep speed), and it composed with the global dock SYNC button: `frame.transport.bpmSync` also had to be on (the contract's "two toggles silently fighting"). Nothing about the beats themselves was locked, and the camera did not follow tempo at all.
- (2): Threshold used the 'steady' motion level (position drift 0.04 units, roll 0.15 degrees, bank 1.5) for a 52-unit-wide aisle: invisible at any slider value.
What changed:
- Field and ring: slab yaw limited to about 30 degrees, 20 slabs instead of 14 (field 106-154), LED on BOTH faces of field and ring pieces, and no rank gating for them (only the corridor opens pair by pair). Lit fraction through the field went from 0-3% to 4-22%; frames now show glowing panels flanking the flight.
- Audio mapping (`Cinema2ThresholdReactiveState.ts`): never dark while music plays (open >= 0.62 with energy, breakdown holds 0.5, level >= 0.5; energy normalised against 0.45), section-aware: Visual Director phase 'peak' / section type drop / drop confidence => a sustained `dropHold` with a beat-by-beat strobe (rows that lead each beat flash above white, the others dip; `u_dropHold`, `u_beatFlip`, `u_beatPulse` in the shared emission GLSL), 'building' opens more, 'low'/breakdown/intro/outro dims to half. New rule `peak-sustain` in the preset holds bloom, shafts and floor glare high while the director reports a peak (the existing one-shot `drop` rule still adds the flash).
- BPM Sync (Threshold's own semantics, documented in the class doc and the control description): ON = beat-locked (every beat of the track's beat grid pulses the screens with a bar accent, a light sweep every bar even when no events were detected, tempo-scaled clocks, camera sway and flight locked to the tempo); OFF = free-running (detected hits only, fixed 120 BPM clock, constant flight speed). It no longer also needs the dock SYNC (`Cinema2SyncedMotionClockResolver.resolve(..., { requireHostSync: false })`), so there is no silent second gate.
- Camera (engine change, opt-in for other presets): new `motion.tempo` block (`weave`, `bob`, `roll`, `fov`, `punch`, `flightSpeed`, `referenceBpm`, `minRate`, `maxRate`) and `tempoSync` camera control (compiler-validated). A beat-position clock always advances; when locked it slides toward the track's beat grid (modulo two bars, rate-limited to 1.5 beats/s, so seeks and track changes never snap or jump); flight speed follows bpm / referenceBpm clamped 0.75-1.45 with a 0.6 s smoothing. Threshold now uses 'gentle' motion with drift 0.7 units / roll 0.7 / FOV 0.8, bank 4 degrees, weave 1.8 units over two bars, bob 0.25 per beat, roll rock 1.4, FOV breathing 1.2 per bar, kick punch 2.4 degrees, and the downbeat lean roll is 2.2. Everything scales with Camera Motion (0 = locked off, only the choreographed lean remains).
Measured in real Chrome after the fixes (960x540 / 480x270, realistic beat grid at 100 and 160 BPM): Camera Motion 0 / 1 / 1.5 gives camera x range 0 / 4.1 / 6.1 units, roll range 1.7 / 3.9 / 5.1 degrees, FOV range 0.1 / 3.4 / 4.8 degrees; with BPM Sync ON the flight covers 11.8 units in 6 s at 100 BPM and 18.6 at 160 BPM (rate 0.83 / 1.39), with it OFF 14.1 units at both; screen pulses per 6 s follow the tempo when ON; the dock SYNC on/off gives identical results. Drop vs verse vs breakdown frames on consecutive beats differ clearly (drop: strobing rows above white; breakdown: panels grey but lit).
Tests: 12 new/updated Threshold tests (never-dark levels, sustained drop with strobe flips, grid pulses and per-bar sweeps ON vs event-only OFF, dock-independence, denser field), `Cinema2CameraTempo.test.ts` (10: opt-in, Camera Motion scaling and zero, tempo locking by zero-crossing counts, flight rate and clamps, no jump when toggling sync or changing tempo, kick punch and recovery, free-running without music, compiler validation), the old 'only light drift' assertion replaced. Cinema2 folder: same 15 failing files / 40 tests as before, 648 passing.
Not done / honest limits: verified with synthetic-but-realistic music frames (a real audio file was not analysed in the app; the real energy range, section labels and Visual Director phases may differ, which is why levels are floored and drops use three independent triggers); with a track whose Visual Director never reports 'peak' the sustained bloom/shafts/glare rule does not fire (the strobe and levels still follow the section type and drop confidence); the corridor's white flood near the vanishing point at some moments is the existing look; the smoke is unchanged.

### HUM:N 3D figure rebuild (owner request 2026-09-25, uncommitted until the owner commits)
The owner showed the old HUM:N (a flat, hand-traced 2D drawing: ~100 line segments and 41 triangles in one front view, moved by 2D warps) next to the wanted look (a full low-poly 3D character that
moves, turns and reaches, with hundreds of triangles, gradient fills that change places) and asked for the EXISTING preset to be rebuilt, not a new one. Nine control problems were reported at the same time.
What was built (all under `modules/humn/` unless noted; preset `presets/Cinema2HumNPreset.ts` revision 22, module `hum-n-native-render` version 2):
- `Cinema2HumNMesh.ts`: the figure is GENERATED in code (no shipped model, no licence): head (9 rings x 12 around, faceted features: brow, eye sockets, nose, cheekbones, lips, chin, ears, two eye discs),
  neck, torso, two arms, two open hands (fingers + thumb, drawn 1.3x life size so a reaching hand reads). Triangle soup with per-vertex 2-bone skin, per-triangle stable rank/seed and per-EDGE stable rank + walking
  direction (shared edges agree, so fragmented edges are open from both sides). Density 1 = 942 triangles and density 2 (Mesh Detail Dense) = 2,378. Winding is verified outward (back faces are culled).
- `Cinema2HumNRig.ts`: ten bones, forward kinematics -> one skinning matrix each. Idle body language scaled by Motion Amount (weight shift, spine/head sway, a dip and nod on every beat) plus the gestures
  (reach with an open palm facing the camera, shock recoil, head grab, lunge, look, body turn, nod). The gesture SCHEDULER is the old `Cinema2HumNPerformance.ts` (kept, trimmed to the timeline; its 2D geometry was removed).
- `Cinema2HumNRenderer.ts`: skinned draw with depth. Fragment shader: barycentric edge lines (thickness scales with frame height), per-triangle fill selection (a stable rank + a slowly advancing shift = the filled set
  turns over), styles Solid / Gradient (hue-space blend between three colours, scrolling) / Stripe / Mixed (gradient, stripes, solid, halftone dots), eyes as concentric rings, kick jitter in the vertex shader.
- `Cinema2HumNAutoColor.ts`: the Auto Color palette (key on the circle of fifths, major/minor, brightness, energy sets saturation and spread, section change steps the hue, chord change nudges it, drop swells it; slewed on the
  hue wheel; slow self-drift with no audio). `modules/Cinema2BeatClock.ts` (NEW, shared): a beat-position clock that locks to the track's beat grid (eases toward it, never snaps) or free-runs at 120 BPM, and stands still when paused;
  the camera's tempo motion now uses it too.
- The preset is now a `world` scene with a real engine camera (static rig, `motion.tempo` sway, `motionAmount` and `tempoSync` bound to Motion Amount / BPM Sync), `depth24` targets, environment background bound to Background.
Owner's nine points and what was found / done:
1. One slider: Master Reactivity is gone; Master Intensity (default 0.75) now scales every music-driven behaviour (fill lifts, motion lift, gesture size, glow/edge pulses). At 0 only the user's values show.
2. BPM Sync did nothing: root cause = the module used the shared clock WITHOUT `requireHostSync: false`, so the Audio Dock SYNC silently gated it (same bug as Threshold), and Motion Amount defaulted to 0. Now HUM:N's own switch:
   ON = sway, beat dip/nod, fill turnover and gradient scroll lock to the track's tempo and beat grid, OFF = free-run at 120 BPM. Verified: scroll ratio 150/90 BPM = 1.67 ON, 1.0 OFF, dock ON/OFF identical.
3. Figure Scale now 0.6-2.5 about the middle of the head (was 0.7-1.3 with a safe-frame clamp).
4. Grid Presence and the grid are gone.
5. Gesture Intensity is gone; Auto Performance (default ON now) owns every pose: drops -> reach / shock / head grab / lunge, phrases and sections -> look / turn / scan / recentre, all gated by Master Intensity; OFF = only idle sway.
   Its old take-over metadata (editing a control turned it off) was removed.
6. Facet Fill at 1 = every triangle filled (Stripe / dot triangles keep their black gaps by design). Fill changes over time (turnover) so it is never a fixed set.
7. Flicker and Jitter root cause: the old amplitudes were tiny (flicker added <= 0.10 alpha to 18 faint ghost lines; jitter moved a third of the strokes by 0.012 units) and both defaulted to 0. Now: Flicker flashes a fresh
   handful of triangles into colour on every beat, a large set on the downbeat (and re-rolls the filled set for good), snares flash the eyes; Jitter throws a third of the triangles off the body by up to 7 cm on the kick. Both are direct
   user gates (not scaled by Master Intensity or Auto Performance; the momentum rule that damped them was removed). Defaults 0.5 / 0.4.
8. Auto Color toggle (default ON) at the top of the Palette; the five manual colour controls (Wireframe, Skin Primary/Secondary/Accent, Pattern Ink) use `visibleWhen` so they collapse while it is on (Background stays).
9. Color Shift Amount removed (Auto Color took its job).
Defaults changed on purpose so it reacts out of the box: Master Intensity 0.75, Motion Amount 0.6, Auto Performance ON, Auto Color ON, Facet Fill 0.35, Fragmentation 0.12, Flicker 0.5, Jitter 0.4, Glow 0.35.
Decisions I made that the owner may want to change: reach uses the viewer-left arm; the gesture set is the old four (reach, shock, head grab, lunge); the head is drawn 1.4x for a heroic silhouette; the default camera is a close bust.
Verification: real Chrome frames (the whole runtime, 960x540 and 1080p); the real app view (Inspector shows Auto Color, collapses/expands the manual colours, no retired controls); `npm run test:e2e:cinema2-humn` = 13 real-browser tests
(spec rewritten: `src/test/e2e/cinema2HumNVisualAcceptance.spec.ts`; run locally with `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH=<Chrome>`); vitest: new `Cinema2HumN.test.ts` (production path), `Cinema2HumNMesh/Rig/BeatClockAndColor.test.ts`, ported
`Cinema2HumNReactivity/Finishing/Preset/Performance.test.ts`; the 2D-geometry suites (Gestures, NativeMotion) were deleted with the 2D code. Cost: 1.5-1.8 ms/frame at 1920x1080 on low, medium and high (M3 Pro, glow + trails on).
Not done / honest limits: verified with synthetic music frames (a real track was not analysed in the app); the face reads as a stylised low-poly head, not a portrait (organic detail would need a modelled asset); fingers do not curl (rigid hands);
the head grab puts the hands beside the head, not on it; Auto Color and the poses could be tuned further against real tracks; saved user sessions that stored the removed controls simply ignore them (not migrated).

## GO-TO: the DVYDRM logo as a 3D preset (owner request 2026-09-26, uncommitted until the owner commits)
First logo preset, and the shared logo asset every later logo preset reuses.
- Shared asset `cinema2-dvydrm-logo` (`public/cinema2/models/dvydrm-logo.glb`, 28k triangles, 870 KB, uncompressed, generated-in-house): built by `scripts/cinema2-assets/generate-dvydrm-logo.mjs` from the owner's master SVG (kept at `scripts/cinema2-assets/sources/dvydrm-logo-master.svg`). The script parses the SVG's absolute M/C/Z paths itself (no DOM), samples the Beziers, decides holes by even-odd nesting, extrudes each path with Three's `ExtrudeGeometry` (small bevel whose widest point is exactly the SVG outline, `bevelOffset = -bevelSize`), smooths normals with a 38 degree crease angle and writes the GLB by hand. Three nodes/materials named `outline` (thin outer ring, one hole), `body` (the cloud, six cut-outs) and `star`, all neutral white metal PBR, 2 units wide, centred on the origin, facing +Z. Gradients and colors in the SVG are ignored on purpose (presets tint and light it). To change the shape or depth: edit the script constants, run it, then `npm run assets:build && npm run assets:check`. Id constant: `CINEMA2_DVYDRM_LOGO_ASSET_ID` (`modules/three/Cinema2ThreeAssetManifest.ts`).
- `three-scene` turntable spin (small additive engine feature): instance flag `spin: true`; module parameters `spinTurnSeconds` (one full turn at the 120 BPM reference; 0 = none) and `spinSync` (default true, locks to the beat grid through the shared `Cinema2BeatClock`, so a faster track turns faster and it stops while paused). `cinema2ThreeSpinRadians(beats, turnSeconds)` is the pure function; the bridge applies the rotation after the node's placement matrix.
- Preset `presets/Cinema2GoToPreset.ts` (`drmvyz.cinema2.go-to`, display name GO-TO, keeper, visible; revision 1): Master Intensity + BPM Sync (master controls), Spin Period (24 s), Metalness, Roughness, Clearcoat, Environment Reflection, Camera Motion (design), Haze / Beam / Ground Mist / Floor Reflection / Bloom / Cinematic Finish (effects), Background, Logo Color, Key / Rim / Accent Light (palette; the three light colors are bound through light `controls.color`). Three colored spots (key front-left above, rim and accent high and to the sides so no beam points at the camera), two LED panels, the shipped studio HDR, wet floor, volumetric haze, bloom and finish; the lights trade the stage every two beats and swell on downbeats, scaled by Master Intensity. Static camera with a small tempo-locked drift.
- Verified in real Chrome (M3 Pro) through the production runtime: 0 failed passes and no module diagnostics; the model is true 3D (face-on, edge-on at a quarter turn, back at a half turn); spin measured by silhouette width: at 240 BPM synced the logo is edge-on at 3 s and 9 s and face-on at 6 s and 12 s (twice as fast), with BPM Sync off 240 BPM turns at the reference rate, a 12 s Spin Period turns twice as fast; at 120 BPM the first seconds rotate a little faster while the clock eases onto the beat grid (by design). 1080p whole frame: high 10.6 ms, medium 5.5 ms, low 3.0 ms. Tests: `__tests__/Cinema2GoTo.test.ts` (7). Cinema2 folder: same 15 failing files / 40 tests as before, 648 passing.
- Lessons: the first tuning had the rim and accent spots behind the logo aimed at the camera, which whited the whole frame out through the haze (haze 0.045, beams 2); side/high lights, haze 0.018 and beam 1 fix it. The reflective floor's screen-space reflection of a floating object is speckled/streaky (roughness 0.5 and reflectivity 0.3 keep it subtle); a closer or lower camera, or the object nearer the floor, would show it better.
- Not done / limits: all three parts share one set of material overrides (Roughness/Metalness/Logo Color apply to every part, so the baked per-part roughness 0.22/0.16/0.10 is flattened as soon as Roughness is bound); per-part looks (gold outline, textured body, facets) need per-part material overrides in the `three-scene` config, which is the next small step; no reveal/assemble animation; no shadows; the SVG is not user-selectable (that is the parked media-slot bridge).

## Parked ideas and the next test scene (discussed 2026-09-25, not started)
### Recommended next preset: the sci-fi blossom chamber (test bed for the Three halves of #9 and #10)
Reference: `~/Downloads/cinema2-3d-reference-images/6-scifi-blossom-chamber.png` (art direction only, an offline render; never promise a pixel match). A dark machinery chamber with a glowing pink/red blossoming tree in the centre, hundreds of hanging cables, a ring with orange LED slits above and a small LED ring platform below, a round backlit window at the top, glossy dark surfaces and a grated floor. Why it is the right test: it needs every piece that is still missing and nothing that was ruled out (no skull/alien/jungle preset).
| In the image | What it exercises |
|---|---|
| Blossom canopy (tens of thousands of petal clusters, per-cluster colour) | Three half of #9: `InstancedMesh`, alpha-cutout cards, per-instance colour/emissive, sway from the engine clock (deterministic, BPM-aware), LOD/impostors for distant clusters, per-tier instance budgets, seeded scatter |
| Hanging cables (hundreds) | Instanced tube segments or seeded curve meshes with sway and LOD by distance |
| Window backlight through the tree | Three half of #10: canopy and cables must cast into the engine shadow map (the service already supports any provider with `renderShadow`; `three-scene` does not implement it yet) so the haze shows dappled shafts; static rest-pose casters let the shadow map be cached |
| Rings, LED slits, glossy machinery | #8 (physical materials, emissive slits, panel lights) - exists |
| Wet dark floor, haze, smoke, bloom, grade | Existing native stack (floor + glare, volumetric + smoke volume, shadows, finish) |
Not achievable as-is (approximate or leave out): petal subsurface glow (fake with emissive backlight), depth of field (the engine has none; a cheap DOF effect would be a separate small task), global illumination (approximate with panel lights + the HDR environment). Biggest 60 fps risks: alpha-cutout overdraw from thousands of cards and the per-frame cost of drawing instances into the shadow map.
Assets: all generated in code with seeded scripts under `scripts/cinema2-assets/` like the concrete, smoke and studio environment (trunk/branches, petal sprite sheet, cables, ring/platform lathe meshes; license `generated-in-house`; no Blender, no outside model, no upload).
Suggested stages (each verified in real Chrome and measured at 1080p before the next): 1) chamber (rings, LED slits, floor, haze, window backlight) from existing systems; 2) canopy via `InstancedMesh` + cutouts + per-instance colour + sway + tier budgets; 3) cables with LOD; 4) Three casters into the engine shadow map (dappled light through the canopy); 5) LOD/impostor tuning across low/medium/high. Start it as an internal preset (tag `internal`, hidden from the list) and make it visible only when the owner is happy. Question still open: should the tree stay pink/red or follow the four palette controls like Threshold?

### Other visual ideas that need the same machinery (instancing, LOD/impostors, cutouts, sway, per-instance attributes, budgets)
1 crystal geode cavern (thousands of faceted crystals, per-instance emissive by band, clearcoat + HDR reflections) · 2 neon city canyon in rain (instanced signs/pipes/windows on both walls, per-instance sign colours, builds on the floor glare and wet concrete) · 3 kelp forest or coral reef (swaying cutout fronds, light shafts need Three shadows) · 4 gothic colonnade/cathedral (repeated ribs, stained-glass panes with per-instance colour, panel lights behind them, shadows) · 5 arena crowd (thousands of silhouettes with phone lights blinking on the beat; far rows as impostors; strongest LOD/impostor case) · 6 asteroid field/debris ring (thousands of rocks, several LODs, per-rock tumble, one shadowed key light; cleanest use of seeded scatter + budgets) · 7 night meadow with fireflies (wind-swept grass blades, deterministic sway, impostors for the far field) · 8 server hall/data-centre aisle (Threshold cousin: racks with blinking per-instance LED states; native instanced boxes cover most of it) · 9 audio-reactive pipe organ/speaker wall (rows of tubes/cones whose scale and emissive follow the spectrum bands; needs a per-instance animation channel) · 10 kinetic light sculpture (hundreds of hanging glass rods/light strips moving in beat-driven waves; per-instance transforms from choreography). Best first consumers if the chamber is not chosen: #5 (LOD/impostors), #6 (seeded scatter + budgets), #9 (audio-driven per-instance animation).

### Logo / wordmark scene and SVG-to-3D (findings, not started)
- The `object3d` module already extrudes raw SVG into a lit 3D mesh, but the SVG is a hard-coded string in the preset's source (`rawSvg` in `Cinema2AtmosphereReferencePreset.ts` and `Cinema2SpatialReferencePreset.ts`). There is NO upload process from an SVG to a 3D object: the Media Source panel (`react/Cinema2MediaSourcePanel.tsx`) lets a user pick a Media Library item for a preset's media slot (image, video or svg) but that path is 2D only, and `object3d` does not read media slots.
- Cinema 2.0 has no "user preset" concept: presets are registered in code (`CINEMA2_FIRST_PARTY_PRESET_DECLARATIONS`), the presets panel lists the registry, and the workspace session stores only parameter state and media descriptors. Two options: (A) a first-party preset built around the owner's logo with the SVG embedded in the repo (can be built and verified now); (B) a real user feature where any user picks an SVG for a preset slot: needs a small bridge (let `object3d` read its SVG from a media slot, re-extrude when it changes, show a clear message when an SVG cannot be extruded) plus decisions on storage. Recommendation: A first, B later reusing the same preset.
- Extruder limits to test against the actual file: filled paths only (gradients, strokes and live text must be outlined first, single colour works best); known bug: the extruder fails on some shapes (for example hexagons, "degenerate triangle"); `object3d` shading is the older path (no PBR, no shadow casting; adding a `renderShadow` caster is small and lets the logo block the haze shafts; full PBR would need the extruded geometry fed into the Three module).
- What would be needed to start: the SVG path, the mood (clean beam stage vs dark and smoky like Threshold), brand colours, and whether it should assemble/reveal on a beat or drop.
...
6. Reference image files are not in the repo: `~/Downloads/cinema2-3d-reference-images/` (re-attach if missing; the sci-fi blossom chamber is saved there as `6-scifi-blossom-chamber.png`); the Threshold reference frames were attached in chat only.
7. Which preset to build next: the sci-fi blossom chamber (recommended, see "Parked ideas"), one of the other nine ideas, or the logo scene (option A first); and for the chamber, whether the tree follows the palette controls.

## Where we are and what happens next
DONE: the HUM:N 3D figure rebuild, #1-#4, the Threshold preset with refinement batches A and B, the #5 Three.js spike (GO), the context-restore engine fix, #6 (`three-scene`), #7 (texture service, floor surface texture, asset pipeline), the native half of #10 (engine shadow map, volumetric and floor consumers, Threshold casting into it), the smoke volume, the native half of #9 and #8 (PBR, HDR environment, panel lights, floor glare). The owner commits; the #8 files were uncommitted when this was written.
NEXT (recommended, owner said 2026-09-25 to pick this back up later): build the sci-fi blossom chamber described in "Parked ideas" as the test bed for the Three halves of #9 (InstancedMesh, LOD/impostors, cutouts, sway, budgets) and #10 (shadows for Three models); the PBR groundwork (physical materials, HDR environment, panel lights) is in place. Other parked directions: the nine other instancing-heavy ideas and the logo scene, both in "Parked ideas". Remaining Threshold reference gaps: per-row floor flash, tower-face shadows, a longer corridor. Open owner decisions: installer budget (50 MB default in force), source-asset location (repo in force), KTX2 vs WebP (WebP in force), runtime camera cuts, whether Threshold becomes the base for more stage presets, and the next-preset choice above.
