...
NEXT (recommended): the Three halves of #9 (InstancedMesh/LOD) and #10 (shadows for Three models) when a sculpted-asset preset (skull, alien, jungle) is started - the PBR groundwork (physical materials, HDR environment, panel lights) is in place; remaining Threshold reference gaps: per-row floor flash, tower-face shadows, a longer corridor only if the floor glare still looks flat. Open owner decisions: installer budget (50 MB default in force), source-asset location (repo in force), KTX2 vs WebP (WebP in force), runtime camera cuts, whether Threshold becomes the base for more stage presets.
