...
- Layout (`modules/threshold/Cinema2ThresholdLayout.ts`): `buildThresholdLayout(seed, { extras })` appends `buildThresholdDetail(seed)` after the unchanged base layout (own seeded PRNG stream, so the base scatter and indices never change): an outer rank of taller dark towers staggered behind the housings (medium and up), overhead structure of cross beams and two longitudinal rails (medium), lower chords, struts and hanging cables (high). All role 0, all above the flight lane (asserted). Instances carry `minTier` (packed in the reserved i3.z float); the vertex shader moves anything above the current tier off screen (main and shadow passes).
- Budgets: `THRESHOLD_INSTANCE_BUDGETS` per tier (boxes / puffs per lap: low 200/30, medium 320/60, high 420/120); the module checks its layout against all three at creation and throws (module fails, engine keeps running) if detail would exceed a tier. Counts today: boxes 160 base + 98 detail; puffs 28 / 42 / 56.
...
Measured in real Chrome, M3 Pro, 1080p, tight loop after warm-up: high 7.7-7.8 ms with detail vs 7.3 ms without, medium 4.8 vs 4.5, low 2.2 both. With extras off the frames match the pre-change baseline to within float noise (0.1% of pixels differ by a few LSB, run-to-run deterministic). Visual: overhead structure reads as the reference's faint ceiling (and casts extra shadows into the key light), smoke pools at the tower bases and billows down the aisle. Context loss: the module stays active and the sprite sheet re-uploads. Tests: layout determinism/tier gating/budgets/lane clearance, smoke ordering and packing, chunk coverage and lap culling, module config validation, per-preset asset budgets now include smokeTexture and Threshold's sprite sheet, pipeline sprite-sheet test. Cinema2 folder: same 15 failing files / 40 tests as before, 616 passing.
