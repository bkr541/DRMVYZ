...
Test state: `npx vitest run src/components/vyzualz/cinema2` = the same 15 failing files / 40 failing tests that pre-date all this work (listed under "What exists today"), 616 passing after native #9; `npm run test:assets` 13 passing; lint and typecheck clean for touched files.
...
NEXT (recommended): #8 only if the floor glare still looks flat (the native floor glare approximation is cheaper), or the Three half of #9 / #10 when a sculpted-asset preset (skull, alien, jungle) is started; the Threshold reference gaps left are puddle glare, tower-face shadows, and a longer corridor only if the floor glare still looks flat. Open owner decisions: installer budget (50 MB default in force), source-asset location (repo in force), KTX2 vs WebP (WebP in force), runtime camera cuts, whether Threshold becomes the base for more stage presets.
